...
import { Activity, ShieldCheck, TrendingUp, BarChart2 } from 'lucide-react';
...
/* ─── Mobile KPI card ──────────────────────────────────────── */
const MobileKPI = ({ label, value, sub, icon: Icon }) => (
  <div className="bg-brand-bg/50 p-4 rounded-2xl border border-white/5 flex flex-col gap-1">
    <div className="flex justify-between items-center mb-1">
      <span className="text-[10px] uppercase font-semibold text-brand-secondary tracking-wider">{label}</span>
      <Icon size={15} className="text-[var(--color-brand-cyan)]" />
    </div>
    <span className="text-2xl font-display font-bold text-[var(--color-brand-cyan)]">{value}</span>
...
const MobileDashboard = () => (
  <div className="flex flex-col gap-4">
    ...
    <div className="grid grid-cols-2 gap-3">
      <MobileKPI label="Eficacia" value="94.2%" sub="Incremento sostenido" icon={Activity} />
      <MobileKPI label="Riesgo" value="Bajo" sub="Sin anomalías" icon={ShieldCheck} />
      <MobileKPI label="Aprobación" value="68%" sub="Índice general" icon={BarChart2} />
      <MobileKPI label="Evolución" value="+24%" sub="vs. año anterior" icon={TrendingUp} />
    </div>

    {/* Area chart */}
    <div className="bg-brand-bg/50 p-4 rounded-2xl border border-white/5">
      <span className="text-[11px] font-semibold uppercase text-brand-secondary tracking-wider block mb-3">Evolución mensual</span>
      <div className="h-[160px] w-full">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={areaData} margin={{ top: 5, right: 5, left: -30, bottom: 0 }}>
            <defs>
              <linearGradient id="colorMetricMobile" x1="0" y1="0" x2="0" y2="1">
            ...
            <Tooltip
              contentStyle={{ backgroundColor: 'var(--color-brand-surface)', borderColor: 'rgba(255,255,255,0.1)', borderRadius: '8px', fontSize: '12px' }}
              itemStyle={{ color: 'var(--color-brand-cyan)' }}
            />
            <Area type="monotone" dataKey="metric" stroke="var(--color-brand-cyan)" strokeWidth={2} fillOpacity={1} fill="url(#colorMetricMobile)" />
          </AreaChart>
        </ResponsiveContainer>
      </div>
    </div>
    ...
    {/* Pie + legend */}
    <div className="bg-brand-bg/50 p-4 rounded-2xl border border-white/5 flex items-center gap-4">
      <div className="h-[100px] w-[110px] flex-shrink-0 relative">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            <Pie data={pieData} innerRadius={32} outerRadius={48} paddingAngle={4} dataKey="value" stroke="none">
      ...
      <div className="flex flex-col gap-2 text-xs">
        <span className="text-brand-secondary uppercase font-semibold tracking-wider text-[10px]">Distribución</span>
        {pieData.map((d, i) => (
...
const DashboardMock = () => {
  return (
    ...
    >
      <div className="absolute -top-32 -right-32 w-[30rem] h-[30rem] bg-[var(--color-brand-cyan)]/10 blur-[100px] rounded-full pointer-events-none" />

      {/* Window chrome */}
      <div className="flex justify-between items-center border-b border-white/10 pb-4 mb-6 md:mb-8 relative z-10">
        <div className="flex items-center gap-2">
          <div className="w-3 h-3 rounded-full bg-red-500" />
          <div className="w-3 h-3 rounded-full bg-yellow-500" />
          <div className="w-3 h-3 rounded-full bg-green-500" />
        </div>
        <span className="text-[10px] md:text-xs text-[var(--color-brand-cyan)] font-mono tracking-widest font-bold">PANEL DE CONTROL AVANZADO</span>
      </div>
      ...
      <div className="hidden md:grid grid-cols-4 gap-6 relative z-10">
        {/* Area Chart */}
        <div className="col-span-2 bg-brand-bg/50 p-4 md:p-6 rounded-2xl border border-white/5 flex flex-col">
        ...
        {/* Pie Chart */}
        <div className="col-span-1 bg-brand-bg/50 p-4 md:p-6 rounded-2xl border border-white/5 flex flex-col">
          ...
          <div className="mt-2 flex justify-center gap-4 text-xs text-brand-secondary">
            <div className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-[var(--color-brand-cyan)]" />Aprobación</div>
            <div className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-slate-700" />Rechazo</div>
          </div>
        </div>
